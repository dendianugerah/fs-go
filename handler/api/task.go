...
func (t *taskAPI) GetTask(w http.ResponseWriter, r *http.Request) {
	...
	task, err := t.taskService.GetTaskByID(r.Context(), int(idTask))
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		log.Println(err.Error())
		json.NewEncoder(w).Encode(entity.NewErrorResponse("error internal server"))
...
func (t *taskAPI) CreateNewTask(w http.ResponseWriter, r *http.Request) {
	...
	_, err = t.taskService.StoreTask(r.Context(), &newTask)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		log.Println(err.Error())
		json.NewEncoder(w).Encode(entity.NewErrorResponse("error internal server"))
...
func (t *taskAPI) UpdateTask(w http.ResponseWriter, r *http.Request) {
	...
	idLogin := r.Context().Value("id")

	idLoginInt, err := strconv.Atoi(idLogin.(string))
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
		log.Println(err.Error())
		json.NewEncoder(w).Encode(entity.NewErrorResponse("invalid user id"))
	...
	var newTask = entity.Task{
		...
		Description: task.Description,
		CategoryID:  task.CategoryID,
		UserID:      int(idLoginInt),
	}

	id, err := t.taskService.UpdateTask(r.Context(), &newTask)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		log.Println(err.Error())
		json.NewEncoder(w).Encode(entity.NewErrorResponse("error internal server"))
